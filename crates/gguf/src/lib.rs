...
//! **The header is not the file.** A quantized checkpoint is routinely several
//! gigabytes, so nothing here loads one: without `std` a header arrives as
//! bytes a host read out of a `File`, a `Blob` or a ranged request, and with
//! `std` the same reader opens a file and reads tensors out of it by range.
//! Either way what you get back is where a tensor lives, not the tensor.
