...
fn push_value(out: &mut String, value: &gguf::Value) {
    use gguf::Value;
    match value {
...
fn array_len(array: &gguf::Array) -> usize {
    use gguf::Array;
    match array {
        Array::U8(v) => v.len(),
        Array::I8(v) => v.len(),
...
fn push_array(out: &mut String, array: &gguf::Array) {
    use gguf::Array;
    ...
    pub fn tensors_json(file: &Header) -> String {
        ...
        for (index, tensor) in file.tensors().iter().enumerate() {
            ...
            out.push_str(&format!(
                ...
                start + tensor.offset,
                tensor.nbytes(),
                gguf_quants::is_supported(tensor.dtype)
            ));
        }
        out.push(']');
        out
...
impl GgufHeader {
    ...
    pub fn strings(&self, key: &str) -> Result<Vec<JsValue>, JsValue> {
        match self.file.metadata().get(key) {
            Some(gguf::Value::Array(gguf::Array::String(items))) => {
    ...
    pub fn tokenizer(&self) -> Result<Tokenizer, JsValue> {
        let strings = |key: &str| -> Result<Vec<String>, JsValue> {
            match self.file.metadata().get(key) {
                Some(gguf::Value::Array(gguf::Array::String(items))) => Ok(items.clone()),
                Some(_) => Err(JsValue::from_str(&format!("{key} is not a string array"))),
        ...
        let number = |key: &str| -> Option<u32> {
            self.file.metadata().get(key).and_then(|v| match v {
                gguf::Value::U32(n) => Some(*n),
                gguf::Value::I32(n) => u32::try_from(*n).ok(),
                gguf::Value::U64(n) => u32::try_from(*n).ok(),
                _ => None,
            })
        };
...
pub struct Tokenizer {
    inner: gguf_tokenizer::Tokenizer,
    bos: Option<u32>,
    eos: Option<u32>,
    pre: String,
}
...
pub fn supported_dtypes() -> Vec<JsValue> {
    ...
    ALL.iter()
        .filter(|dtype| gguf_quants::is_supported(**dtype))
