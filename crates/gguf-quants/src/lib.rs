...
//! right or wrong -- which is why every intermediate here rounds to float32
//! where ggml's does, including the sub-block scale and the product before the
//! minimum comes off. Doing that arithmetic in double and rounding once at the
//! end agrees most of the time and differs in the last bit the rest of it.
//!
//! Nothing here allocates: every routine decodes into a slice its caller
//! already owns, which is what lets a freestanding module with no allocator
//! use the same code a browser does.
