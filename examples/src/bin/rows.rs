...
fn main() {
    ...
    let (path, name) = match (args.next(), args.next()) {
        ...
        _ => { eprintln!("usage: rows <model.gguf> <tensor> [row ...]"); std::process::exit(2); }
    };
    ...
    let info = reader.header().tensor_by_name(&name).cloned().unwrap_or_else(|| {
        eprintln!("no tensor named {name}");
        std::process::exit(1)
    });
    println!("{name} {:?} {:?}, {:.1} MiB in the file",
        info.shape, info.dtype, info.nbytes() as f64 / 1048576.0);

    let started = Instant::now();
    let values = reader.rows_f32(&name, &rows)
        .unwrap_or_else(|e| { eprintln!("{e}"); std::process::exit(1) });
    let elapsed = started.elapsed();

    let width = values.len() / rows.len();
    println!("\n{} rows of {width} in {:.1} ms -- {:.1} KiB decoded, not {:.1} MiB",
        rows.len(), elapsed.as_secs_f64() * 1000.0,
        (values.len() * 4) as f64 / 1024.0, info.nbytes() as f64 / 1048576.0);
    for (i, row) in rows.iter().enumerate() {
        let shown = 6.min(width);
        println!("  row {row:<6} {:?} ...", &values[i * width..i * width + shown]);
    }
}
