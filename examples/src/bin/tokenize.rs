...
use gguf::{Array, GgufFile, GgufReader, Value};
use gguf_tokenizer::{Pattern, Tokenizer};

fn strings(header: &GgufFile, key: &str) -> Vec<String> {
...
fn main() {
    ...
    let (path, text) = match (args.next(), args.next()) {
        ...
        _ => { eprintln!("usage: tokenize <model.gguf> <text>"); std::process::exit(2); }
    };
    ...
    let specials = match header.metadata().get("tokenizer.ggml.token_type") {
        Some(Value::Array(Array::I32(kinds))) => kinds.iter().enumerate()
            .filter(|(_, kind)| **kind == 3 || **kind == 4)
            .map(|(id, _)| id as u32).collect(),
        _ => Vec::new(),
    };

    let tokenizer = Tokenizer::new(tokens, merges, pattern, specials, None);
    println!("vocabulary {} entries, pre-tokenizer {named:?}", tokenizer.vocab_size());
